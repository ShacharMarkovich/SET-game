...
class SetGameHandler:
    ...
    def __init__(self):
        ...
        self.all_cards = []  # all set cards
        self.available_cards = []  # set cards which not had been used so far
        self.board = []
    ...
    @staticmethod
    def check_set(c1: Card, c2: Card, c3: Card) -> bool:
        """get 3 cards, check if is a set
        ...
        Returns:
            bool: whether or not is a set
        """

        def __check_attr(c1_attr, c2_attr, c3_attr) -> bool:
            """check if all the given cards' attribute is the same or all are different"""
            if c1_attr == c2_attr and c1_attr == c3_attr:
                return True
            elif c1_attr != c2_attr and c1_attr != c3_attr and c2_attr != c3_attr:
                return True

            return False

        get_attrs = lambda card:  [getattr(card, attr) for attr in dir(card) if not attr.startswith("__")]

        c1_attrs = get_attrs(c1)
        c2_attrs = get_attrs(c2)
        c3_attrs = get_attrs(c3)

        ans = True
        for i, _ in enumerate(c1_attrs):
            ans = ans and __check_attr(c1_attrs[i], c2_attrs[i], c3_attrs[i])
        return ans
    ...
    def find_set(self):
        ...
        for possible_set in possible_sets:
            if self.check_set(board_cards[possible_set[0]][1],
            ...
            if not new_cards:
                ...
                for frame_key in (k for k in self.window.children.keys() if ',' in k):
                    ...
                    if self.window.children[frame_key].children[child_id].cget('text') == self.PlaceHolder:
                        ...
                        im = self.add_card(f"btn{frame_key}", SetGameHandler.ImagesPath +
                                           new_cards[k].path_2_image, self.window.children[frame_key])
    ...
    @staticmethod
    def __change_state(btn: tk.Button):
        """enable/disable the `btn` state

        Args:
            btn (tk.Button): the button
        """
        ...
        while self.is_active:
            if len(self.selected_btn) == 3:  # must be 3 cards in order to ba a SET
                ...
                for im in self.selected_btn:
                    _cards.append([c for c in self.all_cards if c.path_2_image == im.cget(
                        'text').split('\\')[-1]][0])
                # check if they are a SET:
                if self.check_set(_cards[0], _cards[1], _cards[2]):
    ...
    def flash_color(self, btns: list, color: str):
        """change the given `btns` perimeters with the given `color`

        Args:
            btns (list): tk.Button list
            color (str): color in #<0xRR><0xGG><0xBB> format
        """
        for btn in btns:
